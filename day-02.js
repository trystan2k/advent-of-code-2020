...
function matchRegex(input) {
  ...
  if (result) {
    return {
      minChars: parseInt(result[1]),
      maxChars: parseInt(result[2]),
      char: result[3],
      password: result[4],
    };
  }
...
function resolveStart1(input) {
  ...
  let valids = 0;
  input.forEach((data) => {
    const result = matchRegex(data);
    if (result) {
      const { minChars, maxChars, char, password } = result;
      const charRegex = new RegExp(char, "g");
      const occurencesOfCharInPass = (password.match(charRegex) || []).length;

      if (
        occurencesOfCharInPass >= minChars &&
        occurencesOfCharInPass <= maxChars
      ) {
        valids += 1;
      }
    }
...
function resolveStart2(input) {
  ...
  input.forEach((data) => {
    ...
    if (result) {
      ...
      const charAtOne = result.password.charAt(minChars - 1);
      const charAtTwo = result.password.charAt(maxChars - 1);

      if (
        (charAtOne === char && charAtTwo !== char) ||
        (charAtOne !== char && charAtTwo === char)
      ) {
        valids += 1;
      }
