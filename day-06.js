...
function processAwnsers(person) {
    const values = new Map();
    person.forEach(awnsers => {
        const awnsersValues = awnsers.trim().split('');
        awnsersValues.forEach(val => {
            const curr = values.get(val);
            values.set(val, curr ? curr + 1 : 1);
        });
    });
...
function resolveStar2(input) {
    ...
    return input.map(person => {
        const values = processAwnsers(person);
...
console.log(`For first star, sum of the counts for anyone = ${resultOne.reduce((prev, curr) => prev + curr.size, 0)}`);

const resultTwo = resolveStar2(input);
console.log(`For second star, sum of the counts for everyone = ${resultTwo.reduce((prev, curr) => prev + curr, 0)}`);
