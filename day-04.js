...
function parseInput(inputText) {
    const values = inputText.trim().split('\n\n')
    return values.map(value => {
        const passport = value.trim().split('\n').join('');
        const result = passport.match(regex);
        const obj = {};
        if (result) {
            result.forEach(res => {
...
function resolveStar1(input, reqFields) {
    return input.filter(inp => {
        const keys = Object.keys(inp);    
        return reqFields.every(field => keys.includes(field))
    });
}

function resolveStar2(input, reqFields, validations) {
    const withAllRequiredFields = resolveStar1(input, reqFields);

    return withAllRequiredFields.filter(passport => {
        const keys = Object.keys(passport);
        return keys.every(key => {
            const validationFn = validations[key];
            if (validationFn) {
                return validationFn(passport[key]);
            }
            return false;
        })
    });
